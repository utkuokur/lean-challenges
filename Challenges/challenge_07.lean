...
Given a finite matroid `M` with `n` disjoint bases `B₁,…,Bₙ`, there exist
`n` disjoint bases `C₁,…,Cₙ` such that each `Bᵢ ∩ Cⱼ` is a singleton. -/
...
for any finite matroid `M` with `n` disjoint bases `B₁,…,Bₙ`, and for any
`ε > 0`, there exist `⌈(1 - 1/r - ε)·n⌉` disjoint bases `C₁,…,C_m` such
that each `Bᵢ ∩ Cⱼ` is a singleton.
