...
## Instructions
1. Replace `` in `parameter_1` with a natural number.
2. Replace `` in `challenge_1` with a complete proof, without changing the statement.
...
def r : ℕ := 2 -- The challenge parameter
