...
Given a subset $D \subseteq E$, the \emph{deletion} of $D$ from $M$, denoted $M\setminus D$, is the restriction of $M$ to $E\setminus D$.
The \textit{dual} of $M$, denoted $M^*$, is the matroid with the same ground set as $M$, where a set is independent whenever its complement includes a base of $M$.  Given a subset $C\subseteq E$, the \emph{contraction} of $M$ onto $E\setminus C$ is the matroid $M/C := (M^* \setminus C)^*$.
...
\newpage
\item \textbf{Sidorenko Conjecture for Bipartite Graphs with Bounded Sides.} 
% \cite{ }
...
Note that Sidorenko conjecture is already resolved for half-graphs by \cite{}.
\end{comment}
