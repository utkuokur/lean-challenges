#!/usr/bin/env python3
"""Clean up LaTeXML's HTML output to match the biblatex/PDF look.

1. Bibliographies: LaTeXML renders reference links as "External Links:
   Document, Link" plus a "Cited by:" back-reference block. Rewrite each
   bibitem to the style biblatex prints in the PDF:
   ...
   and drop the "Cited by:" block entirely.

2. Page chrome: remove the "Generated on <date> by LaTeXML" footer (LaTeXML is
   public domain -- the footer is a courtesy credit, not a license requirement;
   a credit comment stays in the HTML source) and strip the timestamp from the
   generator comment so regenerating without source changes is a no-op in git.

Run by scripts/build-html.sh after latexmlc; safe to re-run (idempotent).
"""
...
def strip_chrome(text: str) -> str:
    """Drop the LaTeXML logo footer and de-timestamp the generator comment."""
    # <!--Generated on <date> by LaTeXML (version X) <url>--> : keep the credit,
    # drop the date so unchanged sources regenerate byte-identically.
    text = re.sub(
        r"<!--Generated on [^>]*? by LaTeXML(.*?)-->",
        r"<!--Generated by LaTeXML\1-->",
        text,
        flags=re.S,
    )
    # "Generated on ... by LaTeXML [+ Mascot Sammy]" page footer.
    text = re.sub(
        r'\s*<div class="ltx_page_logo">.*?</div>', "", text, count=1, flags=re.S
    )
    text = re.sub(
        r'\s*<footer class="ltx_page_footer">\s*</footer>', "", text, count=1
    )
    return text
